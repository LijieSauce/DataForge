...
class BingDownloadBatchDialog(QDialog):
    """Bing 批量下载的独立 UI 弹窗。"""

    def __init__(self, parent=None):
        super().__init__(parent)
        # 批量窗口比单图窗口稍高一点，给数量和结果区留出更多空间。
        self.setWindowTitle("Bing 批量下载")
        self.resize(680, 520)
    ...
    def _build_ui(self) -> None:
        ...
        # 顶部说明帮助用户确认这是批量下载窗口，不是单图窗口。
        title = QLabel("Bing 批量下载", panel)
        title.setObjectName("titleLabel")
        description = QLabel("填写 Bing 图片搜索页地址和目标数量，用于批量图片采集。", panel)
